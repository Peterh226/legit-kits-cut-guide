...
SEWING_PROG_FILE = Path(__file__).parent / "sewing_progress.json"
...
def load_sewing_progress() -> dict:
    if SEWING_PROG_FILE.exists():
        return json.loads(SEWING_PROG_FILE.read_text(encoding="utf-8"))
    return {}


def save_sewing_progress(data: dict):
    SEWING_PROG_FILE.write_text(json.dumps(data, indent=2), encoding="utf-8")
...
def api_get_sewing_progress():
    return jsonify(load_sewing_progress())
...
def api_set_sewing_progress():
    ...
    sp = load_sewing_progress()
    sp.setdefault(block_id, {})[str(step_index)] = checked
    save_sewing_progress(sp)
