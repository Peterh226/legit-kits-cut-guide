...
- `config.json` — `quilt_name`, `start_date`, `grid_rows` (string of letter labels e.g. `"ABCDEFGH"`), `grid_cols` (int), `grid_layout` (`"row_letters"` default or `"col_letters"`), `block_orientation` (`"portrait"` default or `"landscape"`); authoritative source for quilt identity and grid size
