...
python lint.py                            # Validate cut_guide_data.py + cross-check vs assembly_data.py & cover metadata; writes quilts/<id>/lint_report.txt
...
The **cut stage** sends three images per page to Claude (the full page plus
left and right halves) to give the vision model higher effective resolution
on small segment-suffix characters. This roughly doubles the per-page image
input token cost but is what catches `B11` / `B1n` style ambiguity.
...
- `cut_guide_data.py` — `DATA`: list of 8-tuples `(fabric_code, fabric_name, sku, fabric_size, piece_num, template_code, quantity, page)`. `piece_num` = Cut # (the circled number on the cut graphic). `template_code` = Segment ID (e.g. `B1a`, `B11`, `C1L` — segment suffixes go a-z, then continue 1, 2, 3...; lowercase `l` is stored as uppercase `L` to disambiguate from digit `1`). `quantity` is misnamed for legacy reasons — it's actually the **sew sequence number** within the segment (position in the sewing order), NOT a count of pieces to cut. Single-piece segments have sew sequence 1.
...
- `lint.py` — validates `cut_guide_data.py`. Checks include: field/quantity/page-number sanity, ambiguous fabric codes, **cover-page cross-check** (extracted vs `config.json` metadata for colors/pieces), **cut-vs-assembly reconciliation** (unknown templates, case-only `L`/`l` mismatches, assembly segments with no cut rows), and **sew-sequence gap detection** (segments where the sew order has holes — likely missed cut rows). Writes a report to `quilts/<id>/lint_report.txt`.
