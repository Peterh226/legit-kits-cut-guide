...
- `config.json` — `quilt_name`, `start_date`, `grid_rows` (string of letter labels e.g. `"ABCDEFGH"`), `grid_cols` (int), `grid_layout` (`"row_letters"` default or `"col_letters"`); authoritative source for quilt identity and grid size
...
`grid_rows`, `grid_cols`, and `grid_layout`. Defaults: `"ABCDEFGH"` / `8` / `"row_letters"`.
